...
'''def hog_image(image_dir=c.IMAGE_DIR):
    ...
    for imgname in imagelist:
        if (imgname.endswith(".jpg")):
            images = cv2.imread(image_dir+imgname)
            images= cv2.resize(images, (368,368), interpolation=cv2.INTER_CUBIC)
    return np.array(images)'''

def test():
    libri = data_catalog()
    filename = 'audio/LibriSpeechSamples/test/'
    raw_audio = read_audio(filename)
    print(filename)
    feature = extract_features(raw_audio, target_sample_rate=SAMPLE_RATE)
    print(filename)
...
if __name__ == '__main__':
   # test()
  # filename = 'audio/LibriSpeechSamples/test/821-8211-0000.wav'
 #  raw_audio = read_audio(filename)
 #  feature = extract_features(raw_audio, target_sample_rate=SAMPLE_RATE)
    preprocess_and_save()