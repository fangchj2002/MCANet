    ...
    def to_inputs(self):
      ...
      #  new_x2 = []
        for i in range(len(self.libri_batch)):
            ...
            x1 = np.load(filename)
            new_x1.append(clipped_audio(x1))
        #    x2_ =cv2.imread(filename.split('.')[0]+".jpg")
        #    x2_ = tf.cast(x2_, dtype=tf.float32) / 255.
        #    new_x2.append(x2_)
        x1 = np.array(new_x1) #(batchsize, num_frames, 64, 1)
     #   x2 = np.array(new_x2)
